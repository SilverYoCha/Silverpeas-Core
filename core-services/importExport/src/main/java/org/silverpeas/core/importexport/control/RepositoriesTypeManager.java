...
public class RepositoriesTypeManager {
  ...
  private static SimpleDocument initializeDocument(final SimpleDocumentPK documentPK,
    ...
    SimpleAttachment attachment = SimpleAttachment.builder(descriptor.contentLanguage)
        ...
        .setTitle(defaultStringIfNotDefined(descriptor.getTitle() , fileName))
        .setDescription(defaultStringIfNotDefined(descriptor.getDescription()))
        .setSize(fileSize)
    ...
    setMetadata(descriptor, document);
    return document;
  }

  /**
   * Sets the metadata from the physical file.
   * @param descriptor the current attachment descriptor.
   * @param document the attachment.
   */
  private static void setMetadata(final AttachmentDescriptor descriptor, SimpleDocument document) {
    if (descriptor.isUseFileMetadata()) {
      ...
      document.setTitle(defaultStringIfNotDefined(document.getTitle(), metadata.getTitle()));
      document.setDescription(defaultStringIfNotDefined(document.getDescription(), metadata.getSubject()));
    }
  }
