 ...
 * @author silveryocha
 */
public class GoogleAvatarSynchroProcess extends AbstractAvatarSynchroProcess {

  private final UserPhoto userPhoto;

  public GoogleAvatarSynchroProcess(final DomainDriver domainDriver,
      final UserPhoto userPhoto, final UserDetail spUser) {
    super(domainDriver::getDomainId, DomainType.GOOGLE, userPhoto::getId, spUser);
    this.userPhoto = userPhoto;
  }

  @Override
  protected void loadPhotoIntoPath(final Path photoPath) throws IOException {
    try (final InputStream body = new ByteArrayInputStream(userPhoto.decodePhotoData())) {
